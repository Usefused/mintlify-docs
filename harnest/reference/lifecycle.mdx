...
sidebarTitle: "Lifecycle reference"
...
| `@context.provider("name")` | Many | Publish a resource | [Application resources](/harnest/runtime/lifecycle/application-resources) |
...
| `@lifecycle.output_policy` | Zero or one | Public narration, tool activity, reasoning, and metadata | [Output policy](/harnest/runtime/lifecycle/output-policy) |
...
| `@lifecycle.storage.tasks` | Zero or one | Durable task queue and results | [Task and cron storage](/harnest/runtime/task-storage) |
| `@lifecycle.storage.cron` | Zero or one | Schedules and atomic occurrence handoff; shares the task provider instance | [Task and cron storage](/harnest/runtime/task-storage) |
