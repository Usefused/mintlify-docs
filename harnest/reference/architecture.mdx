...
| `config.yaml` | Framework, mode, entrypoint, deployment intent, and optional `server` overrides |
...
Deployment execution requires the process opt-in `HARNEST_ENABLE_DEPLOYMENT=true`; it is disabled by default. See [Enable deployment](/harnest/runtime/serving/provisioning#enable-deployment). Rendering a plan remains available without the flag.
