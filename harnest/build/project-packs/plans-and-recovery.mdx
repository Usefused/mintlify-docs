...
Create a new project with `planner.plan_init("support-bot", options={"acme": {"team": "support"}})`. Options are grouped by pack name. This method also accepts `framework="adk"` or `"langgraph"` and `minimal=True`.
...
<Steps>
  ...
  <Step title="Prepare Harnest changes">
    Generate the scaffold or apply core migrations in a disposable copy. The live project remains unchanged.
  </Step>
