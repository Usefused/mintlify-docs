...
The default LLM is `ollama_chat/qwen3.5:cloud`; your local Ollama service must have that model available and cloud access configured. Set `JEV_LLM_MODEL` to another LiteLLM provider/model identifier, with optional `JEV_LLM_API_BASE` and `JEV_LLM_API_KEY` overrides. Ollama reasoning is disabled for this short drafting step.
