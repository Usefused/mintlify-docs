...
The model uses the shared `OPENAI_MODEL`, `OPENAI_BASE_URL`, and optional `OPENAI_API_KEY` process environment. Configure your server's model ID and compatible API URL; no vendor or model is selected by default. Follow [Configure model credentials](/harnest/build/project-configuration#configure-model-credentials) for local, CI, and deployment setup.
