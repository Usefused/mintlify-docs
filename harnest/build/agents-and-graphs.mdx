...
The model uses the shared `OPENAI_MODEL`, `OPENAI_BASE_URL`, and `OPENAI_API_KEY` process environment. Follow [Configure model credentials](/harnest/build/project-configuration#configure-model-credentials) for local, CI, deployment, and OpenAI-compatible backend setup.
