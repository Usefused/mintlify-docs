...
See [Python imports](/harnest/build/python-imports) for the public modules that group agent, model, tool, and runtime APIs.
...
The model uses the shared `OLLAMA_MODEL`, `OLLAMA_BASE_URL`, and optional `OLLAMA_API_KEY` process environment. The default `qwen3.5:cloud` runs through your local Ollama daemon but uses Ollama's cloud. Follow [Configure model credentials](/harnest/build/project-configuration#configure-model-credentials) for local-model, CI, deployment, and explicit OpenAI setup.
...
Use an optional `token_policy` to observe usage, enforce budgets, or choose context reductions per agent. Policies are absent by default and can be disabled or overridden in trusted invocation code. See [Token utilisation](/harnest/build/models-and-libraries/token-utilisation).
