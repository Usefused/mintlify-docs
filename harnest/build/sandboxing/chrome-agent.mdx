...
| Model | A tool-capable model served by an OpenAI-compatible API. Set `OPENAI_MODEL` and `OPENAI_BASE_URL` in `config.yaml`; supply `OPENAI_API_KEY` only if your endpoint requires authentication. |
...
<Steps>
  ...
  <Step title="Set the model credential">
    ...
    Keep secrets out of `config.yaml`, source files, and the browser image. Replace the checked-in `OPENAI_MODEL` and `OPENAI_BASE_URL` placeholders before live calls. The API key is optional for endpoints that do not require authentication.
  </Step>
