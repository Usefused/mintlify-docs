...
Harnest defaults to Ollama for generated agents, eval judges, and text user simulators. They inherit the environment of the same Harnest command; there is no separate eval credential channel. OpenAI is an explicit opt-in, not a fallback.

| Variable | Purpose | Default |
|---|---|---|
| `OLLAMA_MODEL` | Ollama model name | `qwen3.5:cloud` |
| `OLLAMA_BASE_URL` | Ollama API endpoint, without `/v1` | `http://localhost:11434` |
| `OLLAMA_API_KEY` | Optional credential for a protected endpoint | None |
...
Start Ollama before running live model calls. The default `qwen3.5:cloud` model runs in Ollama's cloud through your local daemon and requires Ollama sign-in; a localhost endpoint does not make this model offline. To run a local model instead, pull a tool-capable model supported by your machine and set `OLLAMA_MODEL` to its name in `config.yaml`.

<Note>
  Existing agents are not automatically rewritten. To switch an OpenAI-configured agent to Ollama, change its model to `OllamaModel.from_environment()` and replace `OPENAI_MODEL` and `OPENAI_BASE_URL` with `OLLAMA_MODEL` and `OLLAMA_BASE_URL`. Use Ollama's endpoint without `/v1`. Remove unused OpenAI credential mappings; supply `OLLAMA_API_KEY` only if your Ollama endpoint requires one.
</Note>

For the default cloud-backed model, sign in to your running Ollama daemon before live commands:
...
The local Ollama daemon normally needs no API key. A protected remote endpoint can use `OLLAMA_API_KEY`, exported before the command or injected by the deployment engine.
...
spec:
  ...
  secrets:
    - environmentVariable: OLLAMA_API_KEY
      secretRef: projects/example/secrets/ollama-api-key/latest
```

The deployment engine resolves `secretRef` and injects its value as `OLLAMA_API_KEY`. Local `test`, `run`, and `serve` commands do not resolve `spec.secrets`; export the variable in their process environment even when the deployment mapping exists. The endpoint must be reachable from the deployed agent; `localhost` refers to that agent's own host or container.

### Opt into OpenAI or another provider

Keep `LiteLLMModel.from_openai_environment()` when you explicitly want OpenAI or an OpenAI-compatible endpoint:
...
This helper reads `OPENAI_MODEL` (default `gpt-4.1-mini`), `OPENAI_BASE_URL` (default OpenAI's API endpoint), and `OPENAI_API_KEY`. It normalizes bare model names to the `openai/` prefix. Other native providers remain available through `LiteLLMModel("provider/model", ...)`. Selecting an explicit OpenAI agent does not change omitted eval model IDs: configure judge and simulator IDs explicitly if they should also use OpenAI.
...
When you omit `judgeModelOptions.judgeModel` or the text simulator's `model` from `evals/test_config.json`, Harnest uses `OLLAMA_MODEL` with the same `qwen3.5:cloud` default and Ollama endpoint configuration. Scenario suites use this default even when the entire `userSimulatorConfig` block is absent. An explicitly authored model ID wins. For example, `judgeModel: "gemini-2.5-flash"` opts into ADK's native Gemini provider and requires its credentials, such as `GOOGLE_API_KEY`, independently of the agent's Ollama setup.
...
Neither `OLLAMA_API_KEY` nor `OPENAI_API_KEY` authenticates these native Google services. Keep model and metric selection in source or `test_config.json`; keep every credential in the process environment or deployment secret mapping.
