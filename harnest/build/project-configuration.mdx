...
| `harnest-runtime.lock` | Hash-verified serving resolution | Yes |
| `harnest-test.lock`, `harnest-eval.lock` | Optional test/eval resolutions | When used |
...
```bash
harnest env sync AGENT_DIR --profile test
harnest env sync AGENT_DIR --profile eval
```

The test profile adds pytest and writes `harnest-test.lock`. The eval profile
adds pytest plus Google ADK's evaluation dependencies and writes
`harnest-eval.lock`. `harnest test` and `harnest test --evals` select these
profiles automatically. Commit each lock used by CI. Test/eval locks are not
...
`compile`, `run`, and `serve` select runtime; `test` selects test or eval.
...
harnest env sync AGENT_DIR --profile test --frozen
