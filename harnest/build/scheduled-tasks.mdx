...
When scheduled work invokes the agent without passing an `AgentRuntimePrincipal`, Harnest supplies an empty principal. Permissioned tools and MCP operations therefore fail closed. Construct an explicit service principal in trusted Task code when the schedule needs declared capabilities. See [Agent Runtime Principals](/harnest/runtime/agent-runtime-principals).
