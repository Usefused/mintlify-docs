...
This is shared evaluation infrastructure, not a change to your agent runtime. LangGraph execution and Docker-extension sandboxes do not run through ADK's agent or sandbox executor. Evaluation startup validates the supported ADK version, and Harnest's compatibility CI tests evaluation adapters for both frameworks against pinned, minimum, and latest-compatible dependency sets.
...
ADK evaluation does not require a Google model for every metric. Deterministic metrics need no judge model; compatible LLM judges and text simulators can use your configured model and agent transport. Some service-backed metrics have their own credentials. See [model configuration](/harnest/build/models-and-libraries/configure-a-model#reuse-an-agents-model-client) and the [metric prerequisites](/harnest/build/evaluations/metrics).
