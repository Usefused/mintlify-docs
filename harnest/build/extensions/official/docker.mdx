...
Harnest validates and copies the package without importing its code. Use Python 3.11 or newer with the current Harnest release. Version `0.4.1` requires Harnest `>=0.21.3,<0.24` and `docker>=7.1,<8`. The host must provide a reachable Docker daemon.
