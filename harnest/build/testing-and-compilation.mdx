...
| Runtime Plugin `pyproject.toml` | Joins the same dependency solve and lock |
| Public `@task` | Adds pinned Procrastinate support |

Harnest resolves the complete environment before importing the agent or its Runtime Plugins. Use `harnest env sync AGENT_DIR --frozen` in CI to reject lock drift.
