...
From a managed agent's root, create a server-tool starter without replacing existing code:

```bash
harnest add tool customer-lookup
```

Use `--project <agent-root>` when running the command from elsewhere.
