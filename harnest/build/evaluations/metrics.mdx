...
Harnest passes only the judge's final response to verdict parsing. Provider-marked reasoning and partial response chunks are excluded; a rationale written in the final response is retained. Missing final answers do not become passing verdicts. This is separate from `includeIntermediateResponsesInFinal`, which controls evidence from the agent under test, not the judge.

| Property | Default | Applies to | Purpose |
|---|---:|---|---|
| `threshold` | Required | Every metric | Pass boundary |
| `judgeModelOptions.judgeModel` | `OLLAMA_MODEL`, or `qwen3.5:cloud` | Model-judged metrics | Omit to use the shared Ollama model; explicit IDs select an ADK model provider |
...
Model precedence is an explicit `judgeModel`, then `OLLAMA_MODEL`, then `qwen3.5:cloud`. Use an explicit provider-prefixed model ID when the judge should differ from the agent:
...
```json evals/test_config.json
{
  "criteria": {
    "final_response_match_v2": {
      "threshold": 0.8,
      "judgeModelOptions": {
        "judgeModel": "ollama_chat/my-judge-model",
        "numSamples": 3
      }
    }
  }
...
Replace `my-judge-model` with a model served by your configured endpoint. This explicit ID overrides `OLLAMA_MODEL`. The judge [reuses a compatible agent model transport](/harnest/build/project-configuration#reuse-an-agents-model-client) when one is available, including a lifecycle-owned client, custom gateway, and headers. Ollama uses `OLLAMA_BASE_URL` and optional `OLLAMA_API_KEY`. To opt into OpenAI, use an `openai/` model ID with `OPENAI_BASE_URL` and `OPENAI_API_KEY`; other native providers use their own model IDs and credentials through the same [process environment](/harnest/build/project-configuration#configure-model-credentials).
...
<Tabs>
  ...
  <Tab title="ADK judge model">
    Omit `judgeModel` to use `OLLAMA_MODEL` and the shared `OLLAMA_BASE_URL` and optional `OLLAMA_API_KEY` configuration. The default `qwen3.5:cloud` is cloud-backed, not offline. An explicit `judgeModel`, such as `gemini-2.5-flash`, opts into that ADK model provider and requires its own credentials. Harnest does not replace explicit model IDs.
  </Tab>
