...
Replace `my-judge-model` with a model served by your configured endpoint. This explicit ID overrides `OPENAI_MODEL`. The judge [reuses a compatible agent model transport](/harnest/build/models-and-libraries/configure-a-model#reuse-an-agents-model-client) when one is available, including a lifecycle-owned client, custom gateway, and headers. The shared compatible API uses `OPENAI_BASE_URL` and optional `OPENAI_API_KEY`; other native providers use their own model IDs and credentials through the same [model environment](/harnest/build/models-and-libraries/configure-a-model).
...
The agent under test and every metric backend inherit the same process environment. Follow [Configure a model](/harnest/build/models-and-libraries/configure-a-model) for local, CI, and deployment setup. Metrics may add a second provider to the agent's normal model and tool stack:
...
<Tabs>
  ...
  <Tab title="ADK judge model">
    Omit `judgeModel` to use `OPENAI_MODEL` and the shared `OPENAI_BASE_URL` and optional `OPENAI_API_KEY` configuration. Choose these settings before running live evaluations. An explicit `judgeModel`, such as `gemini-2.5-flash`, opts into that ADK model provider and requires its own credentials. Harnest does not replace explicit model IDs.
  </Tab>
