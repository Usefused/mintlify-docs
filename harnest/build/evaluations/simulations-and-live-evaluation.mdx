...
| `model` | `OLLAMA_MODEL`, or `qwen3.5:cloud` | Generates subsequent user turns; an explicit ADK model ID overrides the shared model |
...
Model precedence is an explicit `userSimulatorConfig.model`, then `OLLAMA_MODEL`, then `qwen3.5:cloud`. For example, add `"model": "ollama_chat/my-simulator-model"` to request another model without changing the agent's model. Text simulators [reuse a compatible agent model transport](/harnest/build/project-configuration#reuse-an-agents-model-client) when available. Ollama uses `OLLAMA_BASE_URL` and optional `OLLAMA_API_KEY`; explicitly selected OpenAI models use `OPENAI_BASE_URL` and `OPENAI_API_KEY`. Other native provider model IDs require their own credentials. Harnest also supplies the Ollama default for scenario suites that omit the entire `userSimulatorConfig` block.
...
|---|---|---|
| Agent under test | `OllamaModel.from_environment()` | Shared `OLLAMA_MODEL`, `OLLAMA_BASE_URL`, and optional `OLLAMA_API_KEY` |
| Text simulator and per-turn judge | Omitted `model` and `judgeModel` | Same Ollama environment; explicit native provider IDs require their own credentials |
...
These calls are live and may consume paid capacity. The default `qwen3.5:cloud` uses Ollama's cloud through the local daemon; choose a local model explicitly for local inference. Follow the shared [model credential process](/harnest/build/project-configuration#configure-model-credentials): supply required credentials before local or CI commands, and use `spec.secrets` only as a deployment mapping. Harnest does not load `.env` files. Never place credentials in eval JSON. Ollama and OpenAI keys do not authenticate Cloud TTS or native Google evaluation services.
