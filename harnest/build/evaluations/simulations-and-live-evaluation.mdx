...
These calls are live and may consume paid capacity. Follow [Configure a model](/harnest/build/models-and-libraries/configure-a-model): export credentials before local or CI commands, and use `spec.secrets` only as a deployment mapping. Harnest does not load `.env` files. Never place credentials in eval JSON. The OpenAI-compatible key does not authenticate Cloud TTS or native Google evaluation services.
