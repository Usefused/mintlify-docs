...
| `model` | Configured `OPENAI_MODEL` | Generates subsequent user turns; an explicit ADK model ID overrides the shared model |
...
Model precedence is an explicit `userSimulatorConfig.model`, then the required `OPENAI_MODEL`; there is no built-in model fallback. For example, add `"model": "openai/my-simulator-model"` to request another model without changing the agent's model. Text simulators [reuse a compatible agent model transport](/harnest/build/models-and-libraries/configure-a-model#reuse-an-agents-model-client) when available. The compatible API uses `OPENAI_BASE_URL` and optional `OPENAI_API_KEY`. Other native provider model IDs require their own credentials. Harnest also supplies the shared configuration for scenario suites that omit the entire `userSimulatorConfig` block.
...
|---|---|---|
| Agent under test | `LiteLLMModel.from_openai_environment()` | Shared `OPENAI_MODEL`, `OPENAI_BASE_URL`, and optional `OPENAI_API_KEY` |
| Text simulator and per-turn judge | Omitted `model` and `judgeModel` | Same compatible API environment; explicit native provider IDs require their own credentials |
...
These calls are live and may consume paid capacity at your configured endpoint. Follow [Configure a model](/harnest/build/models-and-libraries/configure-a-model): choose the model and endpoint, supply required credentials before local or CI commands, and use `spec.secrets` only as a deployment mapping. Harnest does not load `.env` files. Never place credentials in eval JSON. Compatible API keys do not authenticate Cloud TTS or native Google evaluation services.
