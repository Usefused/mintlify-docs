...
| Ambiguous agent model transport | Select an eval model ID with one exact transport match, or remove conflicting same-provider transport choices; see [transport selection](/harnest/build/models-and-libraries/configure-a-model#reuse-an-agents-model-client) |
