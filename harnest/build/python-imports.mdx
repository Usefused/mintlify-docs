...
current managed environment. When Harnest owns that link, it also sets
`python.defaultInterpreterPath` to `${workspaceFolder}/.venv/bin/python` in
`.vscode/settings.json` if that setting is absent. On Windows, it uses
`${workspaceFolder}/.venv/Scripts/python.exe`. VS Code-compatible editors such
as Antigravity IDE and Windsurf use this shared workspace file. Python language
tools alongside the Codex IDE extension use the interpreter selected by its host
editor. An existing selection may need to be changed once to `.venv/bin/python`
because the Python extension uses the default only before it selects an
interpreter for that workspace. PyCharm and other Python
IDEs can detect the `.venv` path or select it directly. The editor can then
resolve `harnest` and show completion, types, and hover documentation.

Codex desktop, Codex CLI, and Antigravity 2.0 are separate agent applications.
For Python commands in these agents, use `.venv/bin/python` directly; their
terminal commands do not inherit the VS Code Python interpreter setting.
