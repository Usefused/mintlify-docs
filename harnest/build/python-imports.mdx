...
environment, point your editor at that environment instead of a system Python.

```bash
harnest env sync support-agent
```

The command prints `Agent environment ready:` followed by the exact Python
executable. Select that executable as the project interpreter in VS Code,
PyCharm, or another Python IDE. The editor can then resolve `harnest` and show
completion, parameter and return types, and hover documentation without a
...
Run `env sync` and update the editor selection again after dependency changes;
the content-addressed environment path can change when the resolved runtime
changes.
