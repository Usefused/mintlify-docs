...
Harnest filters discovered remote tools against the active [Agent Runtime Principal](/harnest/runtime/agent-runtime-principals) and checks the requirement again when either the model or trusted code calls the tool. The MCP server or its gateway remains responsible for authorizing the remote request.
