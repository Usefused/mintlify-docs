...
The Docker extension can also create an extension-owned bridge with bounded service containers, internal DNS aliases, readiness checks, and independent budgets. It never publishes declared service ports on the host. See [Docker extension](/harnest/build/extensions/official/docker#add-service-containers) for the topology API and cleanup contract.
