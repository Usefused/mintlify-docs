...
If the application has multiple PostgreSQL stores, set `HARNEST_TASK_DATABASE_URL` explicitly. Every replica can run a worker against the shared queue. The Task runtime opens its own Procrastinate connection pool even when it derives the DSN from a `PostgresStore`; `lease_pool_options` controls session execution leases, not Task worker concurrency. Set `HARNEST_TASK_DATABASE_URL` to place Task traffic on a separately provisioned database.
