...
|---|---|
| Queue backend | Harnest workers with an explicit `lifecycle.storage.tasks` provider; legacy Procrastinate otherwise |
| Installation | `harnest_postgres` and `harnest_redis` are bundled with Harnest; install custom adapters separately. Legacy dependency added only without an explicit provider |
| Database | Selected by the provider; the legacy path uses `HARNEST_TASK_DATABASE_URL` or one unambiguous `PostgresStore` |
...
For a new application, configure [Task and cron storage](/harnest/runtime/task-storage)
to use PostgreSQL, Redis, or a custom database. Every replica can run a worker
against the same provider. The following commands apply only to the retained
legacy Procrastinate path. It opens its own pool; set
`HARNEST_TASK_DATABASE_URL` explicitly if there are multiple PostgreSQL stores.
...
Call the decorated function directly to unit-test its business logic without a database. Exercise `.defer()`, retries, cancellation, and durable resume through a compiled smoke test with your chosen persistent provider so the real queue and checkpoint boundaries run together. Custom providers can use the [shared conformance suite](/harnest/runtime/task-storage#test-a-custom-adapter).
