...
From the agent folder, run one turn in its compiled runtime:

```bash
harnest run . "Summarize the open incidents"
```
...
printf '%s\n' 'Continue the incident review' \
  | harnest run . --session incident-review
```
...
```bash
harnest compile . --output .harnest/support-agent
