...
| Scheduled or retryable Python work | [Create a Task](/harnest/build/queued-tasks) |
| External workflow or job service | [Harnest Extension](/harnest/build/extensions) |
...
Install and lock it before importing the compiler-owned namespace:

```bash
harnest extensions install hatchet --project support-agent
...
checkpoint retention period, explicitly combine `agent_metadata="raw"` with
