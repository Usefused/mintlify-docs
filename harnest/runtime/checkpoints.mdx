...
A checkpointer owns private execution state and the bounded public completion
snapshot for a durable invocation. It is separate from the [session store](/harnest/runtime/session-storage), which owns committed conversation history and application session data.
...
| Completion snapshot | Return final output, agent metadata, and usage consistently from any replica |
...
Every application resolves exactly one checkpoint factory across its root and Harnest Extension extensions. This example assumes the shared `PostgresStore` from `lib/state.py` in the [storage overview](/harnest/runtime/checkpoints-and-storage):

```python lifecycle/checkpoints.py
