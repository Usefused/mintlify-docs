...
sidebarTitle: "Checkpoints and storage"
description: "Choose session and checkpoint storage for development and production."
---

Every compiled agent needs one session store and one checkpointer. Multimodal agents also use an asset store for session-owned media.
...
The default `MemoryAssetStore` is for development. Configure one root `@lifecycle.asset_store` factory for durable media storage. See [Typed multimodal contracts](/harnest/build/models-and-libraries/typed-multimodal-contracts).
