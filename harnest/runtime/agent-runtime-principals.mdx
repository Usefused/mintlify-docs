...
An `AgentRuntimePrincipal` establishes a default-deny boundary for Harnest-governed capabilities during one invocation. A server tool, client-hosted tool, or MCP operation is available only when it declares a permission carried by the principal. Create the principal in trusted application code after authenticating the caller or deciding which service identity a job should use.
...
Every capability that should remain available under a principal must declare a permission. Server and client-hosted tools accept one permission identifier:
...
| Situation | Behavior |
|---|---|
| Root invocation omits `agent_principal` | Compatibility mode; the existing capability surface remains available unless a custom route uses `server.agentPrincipal: required` |
| Root invocation receives an empty principal | Every Harnest-governed capability is unavailable |
| Active principal encounters an untagged capability | Capability is unavailable |
...
| A restricted invocation suspends on external work | Persists a private versioned permission-name snapshot and reconstructs a fresh principal on the resuming replica |
