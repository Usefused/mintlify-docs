...
`spec.resources` and `spec.scaling` are silently ignored in agent configuration. They never override `server.http` and do not need to be removed for an existing project to load. See [retired settings](/harnest/build/project-configuration#retired-resource-and-scaling-fields) when cleaning up old configs. Configure deployment resource limits and replicas in [`harnest-deployment.yaml`](/harnest/runtime/serving/provisioning); configure standalone HTTP limits here.
