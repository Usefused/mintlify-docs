...
Harnest recompiles each stable source change into a fresh artifact. For a managed
agent environment, the reload supervisor keeps its compiler process warm, so
subsequent changes reuse imported Harnest and framework dependencies while the
agent graph itself is still rebuilt and validated. It never modifies a running
ADK or LangGraph graph in place.
...
<Steps>
  ...
  <Step title="Prepare a generation">
    Dependency changes synchronize a fingerprinted environment first and replace the warm compiler when its interpreter or configured environment changes. Harnest then validates and compiles a new immutable artifact while the current server remains available.
  </Step>
  <Step title="Replace the process">
    After compilation succeeds, Harnest asks the current process to shut down gracefully and starts the new generation. Lifecycle resources, Harnest Extensions, MCP Clients, and storage connections close and reopen through their normal hooks.
  </Step>
</Steps>
