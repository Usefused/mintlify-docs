...
The adapter uses the latest user text. Omit `threadId` for a new session, or supply an existing session ID. It streams text, tool events, and LangGraph state updates. Approval, client-tool, and external-continuation handling is not yet implemented in this adapter; use `/responses` or `/live` for those workflows. Disable the route with `agui_enabled=False` when embedding `create_neutral_app` or `create_neutral_router`.
