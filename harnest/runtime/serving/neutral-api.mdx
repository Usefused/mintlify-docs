...
The binding also supports explicit task lookup, filtered listing, cancellation, and subscriptions. Streaming A2A tasks carry reasoning as working-status messages and portable lifecycle details under `metadata.harnest`; final answer text remains a response artifact. See [Agent2Agent (A2A)](/harnest/runtime/a2a) for serving, outbound clients, security, durable `@task` integration, and replica behavior.
...
| Provider-exposed reasoning text | `thinking` | `response.thinking.delta` |
...
`thinking` is observable agent activity. It is not part of `outputText`, the committed conversation transcript, or evaluation answer text. Providers do not always expose readable reasoning, so a response may contain no thinking events even when reasoning is enabled.
...
`OutputPolicy(agent_metadata="raw")` to add a `raw` mapping to each metadata
