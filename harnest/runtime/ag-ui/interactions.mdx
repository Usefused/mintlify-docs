...
[`@client_input`](/harnest/build/agent-tools/client-tools#private-client-input) ends the run with an interrupt whose `reason` is `private_input`. Its `responseSchema` describes the private form, and `metadata.harnest.privateInput` is `true`.
