...
|---|---|---|
| `--bucket` | Bucket that owns the credentials; omit it to use the default bucket | `fused-cli secret set jira --bucket prod-credentials --type oauth --auth-name JiraOAuth --interactive` |
| `--value-stdin` | Reads the complete client pair from stdin | `printf '%s' 'client_id=…;client_secret=…' \| fused-cli secret set jira --type oauth --auth-name JiraOAuth --value-stdin` |
| `--interactive` | Prompts for both fields | `fused-cli secret set jira --type oauth --auth-name JiraOAuth --interactive` |
| `--type` | Selects `oauth` or `oidc` | `fused-cli secret set jira --type oauth --auth-name JiraOAuth --interactive` |
| `--auth-name` | Selects the exact Registry auth scheme | `fused-cli secret set jira --type oauth --auth-name JiraOAuth --interactive` |
