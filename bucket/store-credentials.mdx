...
A service without its own `bucket:` resolves through the app's top-level default. Webhook secret references still resolve through that default, and an `auth.ref` may only join two services that resolve to the same bucket.
