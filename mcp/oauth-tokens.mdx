...
fused-cli mcp token generate support-agent acme-only \
  --expires-in 1h \
  --fixed-binding '9f2c1e04-…,OAuth2,user_123,7d41a8b2-…'
```

The tuple is `service-id,auth-name,end-user-ref[,resource-id]`. Repeat the flag once per service the token may reach — different services on one token may intentionally use different end-user references.

The CLI rejects malformed UUIDs. The Engine rejects duplicate or unavailable bindings atomically, so a partially-bound token is never issued.
...
|---|---|---|
| `--fixed-binding` | Pins `service-id,auth-name,end-user-ref[,resource-id]`; repeatable | `fused-cli mcp token generate support acme --fixed-binding '9f2c…,OAuth2,user_123'` |
