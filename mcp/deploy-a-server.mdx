...
<Card title="Describe the capabilities you need" icon="wand-magic-sparkles" href="/app/from-a-prompt">
  Use `fused-cli describe` to create an MCP App, update an existing local config, or draft a sequential Unified Operation for review.
</Card>

For supported OAuth integrations, [use a Managed service](/workspace/use-managed-service) or [configure your own provider app](/bucket/connect-user-accounts). Managed auth uses the same MCP connection flow, while Fused keeps the provider client secret on its broker.
...
Omit the method flags to create one App with SDK, MCP, and REST delivery. For MCP only, pass `--mcp`. With `--no-input`, also pass `--description` and an explicit `--operation` or `--select-all` for every service.
