...
Each service resolves credentials through its own `bucket:` override when one is set, otherwise it falls back to the top-level `bucket:`. An `auth.ref` may only join two services that resolve to the same bucket.
