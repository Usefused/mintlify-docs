...
If the selected bucket is missing a required static credential, the Engine returns HTTP `409` with code `bucket_credentials_missing` before contacting the provider. Its structured details identify the service, bucket, authentication type, and scheme and include the exact value-free `fused-cli secret set` command to run before repeating the request.
