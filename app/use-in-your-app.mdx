...
`sdk download <name@version>` extracts the package to `fused-sdks/<name>`. The `sdk apply --download` convenience form does the same after an explicit apply. The package is not published anywhere, so install it from that path.
