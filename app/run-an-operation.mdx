---
title: "Call an app from the CLI or REST"
sidebarTitle: "CLI and REST calls"
description: "The two ways to call an operation without importing the generated package."
---

Most calls should come from the generated package — that is what it is for, and [using it in your app](/app/use-in-your-app) covers it. This page is the other two routes, and the only place `sdk invoke` is documented.

| Route | Use it for | Not for |
|---|---|---|
| `fused-cli sdk invoke` | A one-off check from your terminal: credentials, scope, routing | Anything your application does. Do not shell out to it |
| `POST /v1/apps/{id}/executions` | Production traffic from a language with no generated package | Nothing — this is a real transport |
...
This is the runtime route: one REST endpoint on the Engine's HTTP port, and the only runtime REST route the Engine exposes.
