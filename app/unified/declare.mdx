...
<Tip>
  To draft a sequential operation from a goal, use [`fused-cli describe`](/app/from-a-prompt#describe-a-sequential-operation). It resolves exact provider operations and shows the proposed order, inputs, mappings, and output before apply. You can still author the full configuration directly below.
</Tip>
...
Plan validates the local config before its first Engine request, so one command covers both the recursive output contract and the remote change preview:

```bash
# SDK
fused-cli sdk plan -f .fused/sdks/issues.yaml --no-input --json

# MCP
fused-cli mcp plan -f .fused/mcps/issues.yaml --no-input --json
```

Use the standalone `validate` commands only for offline checks that must not contact an Engine.
