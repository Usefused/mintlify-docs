...
| `--json` | Returns the one-time token, expiry, and metadata as an object | `fused-cli sdk token generate support-sdk ci --no-input --json` |

Use `--no-input --json` in automation. Do not parse the token out of human-readable apply text.
