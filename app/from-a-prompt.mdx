---
title: "Create or update from a prompt"
description: "Describe the capabilities you need, review exact operations, and publish an SDK or MCP version."
---
...
<Note>
  `prompt` requires an interactive terminal. For unattended automation, use [explicit init, extend, or plan/apply commands](/advanced/control-the-workflow).
</Note>
...
Prompt updates are additive. To remove capabilities, change routing or language, upgrade service pins, or replace an existing Unified Operation, [edit and plan the config explicitly](/advanced/control-the-workflow).

## Describe a sequential operation

When your goal requests ordered provider calls as one operation, prompt can draft a [Unified Operation](/app/unified/declare):
