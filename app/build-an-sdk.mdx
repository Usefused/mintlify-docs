...
If activation is needed, init shows one combined confirmation and applies the workspace change under its own receipt. It then builds and validates the SDK candidate in memory, securely offers to fill missing credentials in the selected bucket, and plans the SDK. Only a successful plan creates `.fused/sdks/support-sdk.yaml`; init then applies the SDK, downloads the package, and finishes with first-call guidance. Workspace and SDK changes retain separate receipts.
...
<Card title="Store the credentials" icon="key" href="/bucket/store-credentials">
  Terminal plans can securely repair `bucket_credentials_missing` and retry once. Automation receives the typed error and can pre-provision the exact bucket.
</Card>
