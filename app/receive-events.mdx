...
That last check is Engine-side only. The CLI can confirm `webhook_attachment` is non-empty, but it has no store access to look the config up — so a name that was never applied passes local validation and fails at the Engine. That is deliberate: it fails loudly rather than never delivering.

`webhooks_select_all: true` is the webhook counterpart to `select_all` for operations. The two are independent — you can select all events and only some operations, or the reverse.

Delivery is scoped to the attachment. The bridge resolves which webhook config a connecting SDK attached to from its own applied config, so two registrations for the same service and event never cross-deliver.
