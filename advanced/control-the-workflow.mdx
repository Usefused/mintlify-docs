...
| Situation | Use |
|---|---|
| Creating one App with SDK, MCP, and REST delivery | `fused-cli init <name>` with its services; choose operations when prompted |
| Creating only an SDK, REST API, or MCP server | `fused-cli init <name> --sdk`, `--rest`, or `--mcp` |
...
For App initialization, automation must declare the operation boundary that the terminal selector would normally collect. No method flag creates the combined App:

```bash
fused-cli init support-app \
  --service linear \
  --operation 'linear=issueUpdate' \
  --no-input
```

Use `--select-all linear` instead when the App intentionally needs the service's complete operation surface. Add `--sdk`, `--rest`, or `--mcp` only when you want one delivery method.
...
- If generated `init` (including the default combined App) receives `generation_contract_pin_unavailable`, it first resolves every selected active workspace version. It then prints `Refreshing immutable SDK generation snapshot for <service>@<version>...` as it refreshes each exact snapshot and retries the unchanged plan once. `--no-input` uses the same deterministic path without prompting. The CLI never derives a pin locally, substitutes the runtime contract hash, selects a newer version, or retries twice. REST-only mode, MCP-only mode, ordinary `sdk plan`, and unrelated failures do not refresh snapshots.
