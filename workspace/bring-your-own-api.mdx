...
<Note>
  In automation, pass `--no-input`, then make both decisions explicit: use `--all` or repeat `--select 'GET:/invoices'`, and either repeat `--accept-proposal <id>` or pass `--reject-enrichment`. The command fails instead of opening or waiting for browser review.
</Note>
...
| `--public` | Publishes a new service and its first version, or stages a new version of an existing service as public | `fused-cli import plan ./api.json --name X --slug x --public` |
| `--category` | Category for a new service | `fused-cli import plan ./api.json --name X --slug x --category payments` |
| `--json` | Returns the raw plan response and still saves the receipt for `import apply` | `fused-cli import plan ./api.json --name X --slug x --no-input --json` |
...
| `--json` | Prints the plan-ready snapshot for CI or an agent | `fused-cli import discover --url … --name X --slug x --all --reject-enrichment --no-input --json` |
...
<Warning>
  Import plan and apply allow 20 minutes by default. If apply times out, its outcome is unknown — the Registry may have committed before the response was lost. Do not replay the receipt. Run `fused-cli import status <operation-id>` with the plan ID from the receipt, then follow its returned recovery guidance.
</Warning>
...
<Card title="Build an SDK with it" icon="cube" href="/app/build-an-sdk">
  A successful Engine-mediated import apply also activates the exact imported version in your workspace. Select it directly when you create the SDK.
</Card>
