...
await sdk.Linear.issueUpdate({ id: 'ISS-42', state: 'done' });
```
...
Services sit on the client in PascalCase, and operations follow the provider's own grouping — so `issueUpdate` may be `sdk.Linear.issues.update` instead. The package's generated `README.md` lists the exact paths for the operations *you* selected.
