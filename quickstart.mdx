...
If activation is needed, the CLI shows one combined confirmation. It keeps governance intact by writing separate workspace and SDK receipts. If the selected bucket is missing a provider credential, the CLI securely asks for it and retries planning once.
...
<Note>
  Automating this? Pre-provision credentials with `--no-input --value-stdin`, then run `fused-cli init support-sdk --sdk --service linear --select-all linear --no-input`. Use an explicit `--operation` instead when the app needs a narrower surface. The command fails rather than guessing its mode, an ambiguous service, or a capability boundary. For structured plan and apply output with independent retry boundaries, use the [advanced controlled workflow](/advanced/control-the-workflow).
</Note>
