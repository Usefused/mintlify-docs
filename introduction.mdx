...
In a terminal, omit interaction flags. Fused asks only when it needs a selection, confirmation, or missing credential. In automation, pass `--no-input` so the same command fails with a structured remediation instead of waiting for a person; add `--json` when the command supports machine-readable output.
